...
def _find_windows_rscript() -> Path | None:
    ...
    for install_dir in _find_windows_r_install_dirs():
        for candidate in (
    ...
    for root in _find_windows_r_roots():
        if not root.exists():
            continue

        for candidate in (
            root / "bin" / "x64" / "Rscript.exe",
            root / "bin" / "Rscript.exe",
        ):
            normalized = candidate.as_posix().lower()
            if normalized in seen or not candidate.is_file():
                continue
        ...
        for pattern in ("R-*\\bin\\x64\\Rscript.exe", "R-*\\bin\\Rscript.exe"):
    ...
    if not candidates:
        return None
    candidates.sort(key=_windows_rscript_sort_key, reverse=True)
    return candidates[0]


def _find_windows_r_roots() -> list[Path]:
    roots: list[Path] = []
    env_values = [
        str(Path(os.environ.get("ProgramFiles", r"C:\Program Files")) / "R"),
        str(Path(os.environ.get("ProgramFiles(x86)", r"C:\Program Files (x86)")) / "R"),
        str(Path(os.environ.get("LOCALAPPDATA", "")) / "Programs" / "R"),
        str(Path(os.environ.get("USERPROFILE", "")) / "AppData" / "Local" / "Programs" / "R"),
        str(Path(os.environ.get("USERPROFILE", "")) / "scoop" / "apps" / "r" / "current"),
    ]

    seen: set[str] = set()
    for value in env_values:
        root = _existing_dir(value)
        if not root:
            continue
...
def _find_windows_r_install_dirs() -> list[Path]:
    try:
        import winreg
    except ImportError:
        return []
    ...
    for hive, subkey in registry_keys:
        ...
        try:
            with winreg.OpenKey(hive, subkey) as key:
                install_paths.extend(_registry_install_paths(key, winreg))
        except OSError:
            continue

        for value in install_paths:
            install_dir = _existing_dir(value)
            if not install_dir:
                continue
            normalized = install_dir.as_posix().lower()
            if normalized in seen:
                continue
            seen.add(normalized)
...
def _registry_install_paths(key: Any, winreg: Any) -> list[str]:
    paths: list[str] = []
    ...
    for index in range(subkey_count):
        try:
            ...
            with winreg.OpenKey(key, child_name) as child_key:
                child_install_path, _ = winreg.QueryValueEx(child_key, "InstallPath")
                if child_install_path:
                    paths.append(str(child_install_path))
        except OSError:
            continue
...
def _windows_rscript_sort_key(path: Path) -> tuple[tuple[int, ...], bool, str]:
    version = (0,)
    match = re.search(r"[\\/](?:R|r)-(\d+(?:\.\d+)*)[\\/]", path.as_posix())
    if match:
        version = tuple(int(part) for part in match.group(1).split("."))
    return (version, "\\bin\\x64\\rscript.exe" in str(path).lower(), path.as_posix().lower())
