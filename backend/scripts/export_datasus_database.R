...
ensure_r_packages <- function(packages, repos = "https://cloud.r-project.org") {
  ...
  if (length(missing) > 0) {
    ...
    if (length(cran_missing) > 0) {
      tryCatch(
        {
          install.packages(cran_missing, repos = repos, dependencies = NA, lib = target_lib)
        },
        error = function(e) {
          stop(
    ...
    if ("microdatasus" %in% missing && !requireNamespace("microdatasus", quietly = TRUE)) {
      ...
      if (.Platform$OS.type == "windows") {
        # The upstream project documents a prebuilt Windows binary for read.dbc,
        # which avoids pulling the package from source before installing microdatasus.
        if (!requireNamespace("read.dbc", quietly = TRUE)) {
          message("Installing read.dbc Windows binary from Posit Package Manager snapshot...")
          tryCatch(
            {
              install.packages(
                "read.dbc",
            ...
            error = function(e) {
              stop(
                "Failed to install read.dbc binary required by microdatasus on Windows. Details: ",
                conditionMessage(e)
              )
            }
          )
      ...
      if (!requireNamespace("remotes", quietly = TRUE)) {
        install.packages("remotes", repos = repos, dependencies = NA, lib = target_lib)
      }
      tryCatch(
        {
        ...
        error = function(e) {
          stop(
            "Failed to install microdatasus from GitHub fallback. ",
            "On Windows, verify that read.dbc was installed successfully and, if needed, install Rtools.\n",
            "Details: ",
            conditionMessage(e)
          )
        }
